...
export const alt = 'TokenRadar — Real-time Solana Memecoin Radar';
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '40px', marginTop: '48px' }}>
          {[
            { value: '< 5s', label: 'Detection' },
            { value: '24/7', label: 'Monitoring' },
            { value: 'Free', label: 'No Signup' },
          ].map((s) => (
            <div key={s.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <span style={{ fontSize: '28px', fontWeight: 700, color: '#00ff88', fontFamily: 'monospace' }}>{s.value}</span>
              <span style={{ fontSize: '14px', color: '#666', marginTop: '4px' }}>{s.label}</span>
            </div>
          ))}
        </div>
      </div>
